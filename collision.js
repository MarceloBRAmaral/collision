...
document.onkeydown=function(event){ if      (event.keyCode===40){pressingDown=true;}
...
document.onkeyup=function(event){   if      (event.keyCode===40){pressingDown=false;}
...
function Actor(type,color,width,height,speed,xpos,ypos){
    ...
    this.testBounds = function() {
        if(this.xpos>canvasWidth-this.width){this.xpos=canvasWidth-this.width;
        }else if(this.xpos<0){this.xpos=0;}
        if(this.ypos>canvasHigth-this.height){this.ypos=canvasHigth-this.height;
